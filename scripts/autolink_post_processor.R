...
autolink_post_processor <- function(metadata, input_file, output_file, clean, verbose) {
  ...
  local_packages = c(
    ...
    tfruns = file.path(website_relative_path, "tools", "tfruns"),
    cloudml = file.path(website_relative_path, "tools", "cloudml"),
    tfdatasets = file.path(website_relative_path, "tools", "tfdatasets"),
    tfdeploy = file.path(website_relative_path, "tools", "tfdeploy")
  )
