...
# copy package docs
copy_docs <- function(package, packages_dir = "~/packages", to = package, output = c("home", "articles", "examples", "reference")) {
  ...
  if ("home" %in% output) {
    copy_files(
      ...
      to = paste0("content/", to)
    )
    copy_files(
      from = list_images(file.path(package_dir, "docs")),
      to = paste0("static/", to, "/images")
    )
  }
  ...
  if ("articles" %in% output) {
    copy_files(
      ...
      to = paste0("content/", to, "/articles")
    )
    copy_files(
      from = list_images(package_vignettes_dir)   ,
      to = paste0("static/", to, "/articles/images")
    )
    # copy from root images as well to allow root override of vignette images
    copy_files(
      from = list_images(package_dir),
      to = paste0("static/", to, "/articles/images")
    )
  }
  ...
  if ("examples" %in% output) {
    if (file_test("-d", package_examples_dir)) {
      copy_files(
        ...
        to = paste0("content/", to, "/articles/examples")
      )
    }
  }
  ...
  if ("reference" %in% output) {
    ...
    file.copy(package_dir, pkgdown_dir, recursive = TRUE,
              overwrite = TRUE, copy.mode = TRUE, copy.date = TRUE)
    
    # determine the target directory
    reference_path <- normalizePath(paste0("content/", to, "/reference"))
    ...
    current_yaml$destination <- dirname(reference_path)
...
copy_docs("keras")
copy_docs("tensorflow", output = "reference")
#copy_docs("tfruns", to = "tools/tfruns", output = c("articles", "reference"))
#copy_docs("cloudml", to = "tools/cloudml", output = c("articles", "reference"))
copy_docs("tfdatasets", to = "tools/tfdatasets", output = c("articles", "reference"))
#copy_docs("tfdeploy", to = "tools/tfdeploy", output = c("articles", "reference"))
