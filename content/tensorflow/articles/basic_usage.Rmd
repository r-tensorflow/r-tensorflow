...
menu:
  main:
    ...
    identifier: "tensorflow-using-tensorflow-computations"
...
Variables maintain state across executions of the graph. The following example shows a variable serving as a simple counter. See [Variables](howto_variables.html) for more details.
...
A `placeholder()` operation generates an error if you do not supply a feed for it. See the [MNIST fully-connected feed tutorial](tutorial_tensorflow_mechanics.html) ([source code](https://github.com/rstudio/tensorflow/blob/master/inst/examples/mnist/fully_connected_feed.R)) for a larger-scale example of feeds.
